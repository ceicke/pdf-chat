...
class PromptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create prompt" do
    assert_difference("Prompt.count") do
      post prompts_url, params: { prompt: { chosen: @prompt.chosen, prompt: @prompt.prompt } }
    end
  ...
  test "should update prompt" do
    patch prompt_url(@prompt), params: { prompt: { chosen: @prompt.chosen, prompt: @prompt.prompt } }
